...
public class BaoxuiController {
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        //字典表数据转换
        List<BaoxuiView> list =(List<BaoxuiView>)page.getList();
    ...
    */
    ...
    public R save(@RequestBody BaoxuiEntity baoxui, HttpServletRequest request){
        ...
        Wrapper<BaoxuiEntity> queryWrapper = new EntityWrapper<BaoxuiEntity>()
    ...
    */
    ...
    public R update(@RequestBody BaoxuiEntity baoxui, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,baoxui:{}",this.getClass().getName(),baoxui.toString());
        BaoxuiEntity oldBaoxuiEntity = baoxuiService.selectById(baoxui.getId());//查询原先数据
...
//        else if("用户".equals(role))
//            baoxui.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(baoxui.getBaoxuiPhoto()) || "null".equals(baoxui.getBaoxuiPhoto())){
                baoxui.setBaoxuiPhoto(null);
        }

            baoxuiService.updateById(baoxui);//根据id更新
            return R.ok();
    }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<BaoxuiEntity> oldBaoxuiList =baoxuiService.selectBatchIds(Arrays.asList(ids));//要删除的数据
        baoxuiService.deleteBatchIds(Arrays.asList(ids));
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<BaoxuiEntity> baoxuiList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            BaoxuiEntity baoxuiEntity = new BaoxuiEntity();
//                            baoxuiEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            baoxuiEntity.setChongdianzhuangId(Integer.valueOf(data.get(0)));   //充电桩 要改的
//                            baoxuiEntity.setBaoxuiName(data.get(0));                    //报修名称 要改的
//                            baoxuiEntity.setBaoxuiPhoto("");//详情和图片
//                            baoxuiEntity.setBaoxuiTypes(Integer.valueOf(data.get(0)));   //报修类型 要改的
//                            baoxuiEntity.setInsertTime(date);//时间
//                            baoxuiEntity.setBaoxuiZhuangtaiTypes(Integer.valueOf(data.get(0)));   //报修状态 要改的
//                            baoxuiEntity.setBaoxuiContent("");//详情和图片
//                            baoxuiEntity.setCreateTime(date);//时间
                            baoxuiList.add(baoxuiEntity);
                        ...
                        //查询是否重复
                        baoxuiService.insertBatch(baoxuiList);
                        return R.ok();
                    }
                }
            }
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody BaoxuiEntity baoxui, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,baoxui:{}",this.getClass().getName(),baoxui.toString());
        Wrapper<BaoxuiEntity> queryWrapper = new EntityWrapper<BaoxuiEntity>()
            .eq("yonghu_id", baoxui.getYonghuId())
            .eq("chongdianzhuang_id", baoxui.getChongdianzhuangId())
            .eq("baoxui_name", baoxui.getBaoxuiName())
            .eq("baoxui_types", baoxui.getBaoxuiTypes())
            .eq("baoxui_zhuangtai_types", baoxui.getBaoxuiZhuangtaiTypes())
//            .notIn("baoxui_types", new Integer[]{102})
            ;
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        BaoxuiEntity baoxuiEntity = baoxuiService.selectOne(queryWrapper);
        if(baoxuiEntity==null){
